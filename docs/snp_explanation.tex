...
\large Theory and Matrix Representation}
...
\begin{abstract}
This document serves as a technical reference for Spiking Neural P Systems (SN P Systems), a class of distributed parallel computing models within Membrane Computing. It details the formal definition, neuron rules, and the matrix representation method used for computational simulation, specifically focusing on the implementation details found in WebSnapse Reloaded and associated matrix representation literature.
\end{abstract}

\section{Overview}
Spiking Neural P Systems are inspired by the neurophysiological behavior of neurons sending electrical impulses (spikes) along axons to other neurons via synapses \cite. Structurally, an SN P system is a directed graph where nodes represent neurons and edges represent synapses. The system operates under a global clock, synchronizing all neurons to work in parallel, and is known for trading memory for time to solve computationally hard problems \cite.
...
Based on the WebSnapse Reloaded implementation logic, the computation cycle for the next configuration follows these steps \cite:

\begin{algorithm}
\caption{Compute Next Configuration (WebSnapse Logic)}
\begin{algorithmic}[1]
\State \textbf{Input:} $C^{(k)}$, $St^{(k)}$, $Iv^{(k)}$, $M_{\Pi}$, $STv^{(k)}$
\State \textbf{Output:} $C^{(k+1)}$
\State $NG \gets STv^{(k)}$ \Comment{Initialize Net Gain with input spikes}
\State $Delta \gets Iv^{(k)} \cdot M_{\Pi}$ \Comment{Compute net internal spikes (Matrix Mul)}
\State $NG \gets NG + Delta$
\State $MaskedGain \gets St^{(k+1)} \odot NG$ \Comment{Apply Status Mask (Hadamard)}
\State $C^{(k+1)} \gets C^{(k)} + MaskedGain$ \Comment{Update Configuration}
\end{algorithmic}
\end{algorithm}

\section{Note on Non-Determinism and Delays}
\begin{itemize}
    \item \textbf{Non-Determinism:} If multiple rules in a single neuron are applicable, one is chosen non-deterministically to form the Indicator Vector $Iv^{(k)}$ \cite.
    \item \textbf{Delays:} A rule with delay $d$ updates the Delay Status Vector $DSv$. When $DSv_i$ reaches 0, the associated neuron becomes open ($St_i = 1$) \cite.
\end{itemize}
