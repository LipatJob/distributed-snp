...
This section provides a step-by-step execution trace of a specific Spiking Neural P System with delay. This example is derived from the literature \citeand can be used to verify the correctness of a simulator's matrix-vector operations.
...
Based on the rules and synapses, we define the Production Matrix ($PM$) and Consumption Matrix ($CM$). The rows correspond to rules $(r_1, r_2, r_3, r_4)$ and columns to neurons $(\sigma_1, \sigma_2, \sigma_3)$.
...
\begin{itemize}
    ...
    \item \textbf{Selection:} $\sigma_1$ has 1 spike. Rule $r_1$ applies.
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Selection:} $\sigma_2$ fires $r_2$.
